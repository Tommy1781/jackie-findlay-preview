const tracks = [
  {title:'Billy', file:'', duration:'4:06'},
  {title:'Leaders & The Lead', file:'', duration:'3:28'},
  {title:'Awakening', file:'', duration:'4:26'},
  {title:'Incredible Story', file:'', duration:'4:00'},
  {title:'Sweet Djembe', file:'', duration:'3:52'},
  {title:'Wonder', file:'', duration:'4:03'},
  {title:'Heart Song', file:'', duration:'4:36'},
  {title:'Saturday', file:'', duration:'3:11'},
  {title:'Wings of Freedom', file:'', duration:'3:45'},
  {title:'Home', file:'', duration:'4:09'}
];
...
tracks.forEach((t,i)=>{
  ...
  row.innerHTML=`<span class="track-no">${String(i+1).padStart(2,'0')}</span><span class="track-title" title="${t.title}">${t.title}</span><span class="track-time">${t.duration}</span><button class="track-play" aria-label="Play ${t.title}">▶</button><button class="track-buy" data-title="${t.title}">CAD $1.99</button>`;
  row.querySelector('.track-play').addEventListener('click',()=>setTrack(i,true));
  row.querySelector('.track-buy').addEventListener('click',()=>showToast(`Checkout for “${t.title}” (CAD $1.99) will be connected later.`));
...
document.querySelectorAll('.buy-placeholder').forEach(b=>b.addEventListener('click',()=>showToast('Album checkout (CAD $9.99) will be connected after the sales service is chosen.')));
